 ...
 * - NLP Planner: LLM-first (JSON mode) + guardrails + heuristics (Hebrew-aware)
 * - Triggers: gmail.unreplied, gmail.from, sheets.match
 * - Actions: sheets.append, email.send (Gmail), whatsapp.send (Twilio), slack.send, telegram.send, webhook.call
 * - Saved automations per user (by Gmail address)
 *
 * ENV (typical on Render):
 ...
 *
 * NLP providers:
 *  NLP_PROVIDER=heuristic | groq | openai
 *  GROQ_API_KEY=... , GROQ_MODEL=llama-3.1-8b-instant
 *  OPENAI_API_KEY=... , OPENAI_MODEL=gpt-4o-mini
 *
 * Optional integrations:
 *  Twilio WhatsApp: TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_WHATSAPP_FROM, (TWILIO_WHATSAPP_TO optional)
...
function normalizeHe(s = '') {
  ...
  // Common spelling variants (WhatsApp, Sheets, Gmail words)
  t = t.replace(/ווא?טסאפ|ווטסאפ/gi, 'whatsapp');
  t = t.replace(/גוגל\s*שיט|שיט\b/gi, 'sheet'); // משאיר "גיליון" כמילה נפרדת
  t = t.replace(/\bgmail\b/gi, 'gmail');
...
// Regex families (Hebrew-aware)
const RE_GMAIL_WORDS   = /(gmail|מייל(?:ים)?|דוא["”]?ל|אימייל)/i;
const RE_UNREPLIED     = /(שלא\s*נענ(?:ו|ה)|לא\s*נענ(?:ו|ה)|ללא\s*מענה|unrepl(?:ied)?|sla)/i;
const RE_SEND_VERBS    = /(שול(?:ח|חים|חת|חות)|ישל(?:ח|חו)|שלח(?:ו)?|send(?:s|ing)?|נשלח)/i;
const RE_WHATSAPP      = /(whats\s*app|whatsapp|ו[וא]?טסאפ)/i;
const RE_SHEET_WORDS   = /(גיליון|sheet|טבלה)/i;
const RE_TWO_PEOPLE    = /(?:2|שניים|שני|שתיים|שתי)\s*(?:אנשים|שולחים)|אחד\s*מהם/i;
...
// ---------------- Heuristics Planner (Hebrew-aware) ----------------
function heuristicsPlan(textOriginal) {
  const t = normalizeHe(textOriginal);
  const steps = [];
  const missing = [];

  // Case A: SLA - מיילים שלא נענו
  const mentionsMail = has(RE_GMAIL_WORDS, t);
  const mentionsUnreplied = has(RE_UNREPLIED, t);
  if (mentionsMail && mentionsUnreplied) {
    const fromEmail = extractEmail(t);
    const newerThanDays = extractNewerThanDays(t);
    const hours = extractHours(t);

    steps.push({
      trigger: { type: 'gmail.unreplied',
        params: { fromEmail: fromEmail || '', newerThanDays, hours, limit: 50 } }
    });

    if (has(RE_WHATSAPP, t)) {
      steps.push({
  ...
  // Case B: 'טבלה לשני אנשים; כל פעם שאחד מהם שולח מייל'
  const mentionsSheet = has(RE_SHEET_WORDS, t);
  const mentionsTwoSendersPattern = RE_TWO_PEOPLE.test(t) && RE_SEND_VERBS.test(t);
  const mentionsSomeoneSends = /כל\s*פעם\s*שאחד(?:\s*מהם)?\s*שול/i.test(t);

  if (mentionsSheet && (mentionsTwoSendersPattern || mentionsSomeoneSends || RE_SEND_VERBS.test(t))) {
    const emails = extractEmails(t);
    const fromEmails = emails.join(', ');
    const spreadsheetId = extractSpreadsheetId(t) || '';
    const sheetName = extractSheetName(t) || 'InboxLog';

    steps.push({
      trigger: { type: 'gmail.from',
        params: { fromEmails: fromEmails || '', newerThanDays: 30, limit: 100 } }
    });
    steps.push({
      action: { type: 'sheets.append',
        params: {
  ...
  // Case C: Sheets match → Email/WhatsApp
  if (/(google\s*sheets?|גיליון|sheet|לשונית|טאב|row|שורה)/i.test(t)) {
    const spreadsheetId = extractSpreadsheetId(t) || '';
    const sheetName = extractSheetName(t) || 'Sheet1';
    const columnName = extractColumnName(t) || '';
    const equals = extractEqualsValue(t) || '';
...
function humanizeKey(skillName, key) {
  const map = {
    ...
    to:            { label: 'כתובת מייל לקבלת ההתראה', example: 'name@example.com' },
  };
...
// ---------------- LLM (Groq/OpenAI) with JSON mode ----------------
...
דרישות:
- החזר אך ורק אובייקט JSON תקין: { "proposal":[...], "missing":[...] }.
- אל תחזיר טקסט חופשי סביב ה-JSON.
- אם חסרים ערכים, הוסף אותם למערך missing בפורמט: { "key":"...", "label":"...", "example":"..." }.
`;

function safeJsonParse(s) { try { return JSON.parse(s); } catch { return null; } }
function tryExtractJsonBlock(s) {
  const m = s?.match(/\{[\s\S]*\}$/);
  return m ? safeJsonParse(m[0]) : null;
}
function sanitizeProposal(raw = []) {
  const allowedTriggers = new Set(['gmail.unreplied','gmail.from','sheets.match']);
  const allowedActions  = new Set(['sheets.append','email.send','whatsapp.send','slack.send','telegram.send','webhook.call']);
  const out = [];
  for (const step of raw) {
    if (step?.trigger?.type && allowedTriggers.has(step.trigger.type)) {
      out.push({ trigger: { type: step.trigger.type, params: step.trigger.params || {} } });
    } else if (step?.action?.type && allowedActions.has(step.action.type)) {
      out.push({ action: { type: step.action.type, params: step.action.params || {} } });
    }
  }
  return out;
}
...
async function planFromText(text) {
  ...
  if (NLP_PROVIDER !== 'heuristic') {
    try {
      ...
      console.warn('[planner] LLM failed:', e.message || e);
    }
  }

  // 2) נפילה מסודרת להיוריסטיקה
...
async function triggerGmailFrom(params) {
  const { fromEmails, newerThanDays = 30, limit = 100 } = params || {};
